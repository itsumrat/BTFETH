  ...
  <div class="card-body p-0">
    <table class="admin-table">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <div style="font-weight:700;font-size:14px;color:var(--text);">{{ $inv->user->name }}</div>
            <div style="font-size:11px;color:#9ca3af;">{{ $inv->user->email }}</div>
          </td>
          <td>
            <span style="font-size:12px;font-weight:800;padding:4px 10px;border-radius:6px;background:{{ $inv->planBadgeColor() }}22;color:{{ $inv->planBadgeColor() }};border:1px solid {{ $inv->planBadgeColor() }}44;">
              {{ $inv->plan_name }}
            </span>
          </td>
          <td style="font-family:'JetBrains Mono',monospace;font-weight:700;color:var(--text);">${{ number_format($inv->amount, 2) }}</td>
          <td>
            <span style="font-family:'JetBrains Mono',monospace;font-weight:800;color:#f59e0b;">{{ $inv->profit_percent }}%</span>
            <div style="font-size:10px;color:#9ca3af;">≈ ${{ number_format($inv->estimatedProfit(), 2) }}/cycle</div>
          </td>
          <td style="font-size:13px;color:var(--text);">
            {{ $inv->duration_days }} days<br>
            <span style="font-size:11px;color:#9ca3af;">{{ $inv->start_date->format('d M') }} → {{ $inv->end_date->format('d M Y') }}</span>
          </td>
          <td>
            <span style="font-size:13px;font-weight:700;color:var(--text);">{{ $inv->completed_cycles }}/{{ $inv->total_cycles }}</span>
            <div style="font-size:11px;color:#9ca3af;">{{ $inv->daysRemaining() }} days left</div>
          </td>
          <td style="min-width:100px;">
            <div style="background:var(--surface2);border-radius:4px;height:6px;overflow:hidden;">
              <div style="width:{{ $inv->progressPercent() }}%;height:100%;background:linear-gradient(90deg,var(--accent),#22c55e);border-radius:4px;"></div>
            </div>
            <div style="font-size:10px;color:#9ca3af;margin-top:3px;">{{ $inv->progressPercent() }}% complete</div>
          </td>
          <td>
          ...
          <td>
            <div style="display:flex;gap:6px;">
              <button class="btn btn-ghost btn-xs" onclick="openEdit({{ $inv->id }}, {{ json_encode($inv) }})">✏️ Edit</button>
              <form method="POST" action="{{ route('admin.investments.destroy', $inv) }}" onsubmit="return confirm('Delete this investment?')">
                @csrf @method('DELETE')
